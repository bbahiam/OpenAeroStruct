...
prob['delta_aileron'] = 5.
prob['omega'] = np.array([1.,0.,0.])

# View model
#n2(prob)

# Set up arrays to get data
dels = np.linspace(0.,17.5,5)
vels = np.linspace(20,300,5)
Mach = np.ones_like(vels)
q = 0.5*rho*vels**2
#CM_x = np.zeros((len(dels),len(vels)))
p = np.zeros((len(dels),len(vels)))
...
for i,d in enumerate(dels):
    for j,v in enumerate(vels):
        ...
        #CM_x[i,j] = prob['AS_point_0.total_perf.moment.CM'][0]
...
dels_rad = dels*np.pi/180        # Deflection angles in rad
p_deg = p*180/np.pi
